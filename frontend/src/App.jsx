...
function UploadDialog({ onDone }) {
  ...
  const choose = (nextFiles) => { setFiles([...nextFiles]); setResult(null); setError(""); };
  const submit = async () => {
  ...
  return <div className="panel upload-panel"><div className="panel-title"><div><h3>Envio manual</h3><span>somente dados de teste</span></div><CircleHelp size={17} color="#7c918b"/></div><div className="upload-zone" onDragOver={(e)=>e.preventDefault()} onDrop={(e)=>{e.preventDefault();choose(e.dataTransfer.files)}}><UploadCloud size={26}/><p>Arraste os estudos ou selecione arquivos DICOM, PNG ou JPEG.</p><input id="files" type="file" multiple accept=".dcm,.dicom,image/png,image/jpeg" hidden onChange={(e)=>choose(e.target.files)}/><label className="btn btn-subtle" htmlFor="files"><CloudUpload size={15}/>Selecionar arquivos</label></div>{files.length > 0 && <div className="upload-selection">{files.map((f)=><div key={`${f.name}-${f.size}`} className="meta-row"><FileImage size={14}/>{f.name}<span className="mono">{(f.size/1024/1024).toFixed(1)} MB · {busy ? "enviando" : "pronto"}</span></div>)}<button className="btn btn-primary" onClick={submit} disabled={busy}>{busy ? "Enviando…" : "Enviar estudos"}</button></div>}{result?.errors?.length > 0 && <div className="notice upload-result"><strong>{result.studies?.length || 0} aceito(s); {result.errors.length} rejeitado(s).</strong>{result.errors.map((item)=><div key={item.filename} className="meta-row"><span>{item.filename}</span><span>{item.error}</span></div>)}<button className="btn btn-subtle" onClick={onDone}>Voltar à worklist</button></div>}{error && <div className="error-box upload-error">{error}</div>}</div>;
}
...
function Settings() {
  ...
  return <AppShell>
    <div className="page-heading settings-page-heading">
      <div><div className="eyebrow">Sistema / conectividade</div><h1>{data?.wado_anonymous ? "Configuração WADO" : "Configuração DICOMweb"}</h1><p className="page-subtitle">{data?.wado_anonymous ? "Endpoint WADO de entrada para o OsiriX." : "Endpoints de pesquisa e entrada para dispositivos autorizados."}</p></div>
    ...
    {error ? <div className="error-box settings-error"><Wifi size={22}/><h3>Configuração indisponível</h3><p>{error}</p><button className="btn btn-subtle retry-button" onClick={load}>Tentar novamente</button></div> : !data ? <div className="settings-loading"><div className="skeleton"/><div className="skeleton"/><div className="skeleton"/></div> : <div className="settings-grid">
      ...
      <SettingsCard className="settings-card-stow">
         <div className="settings-card-header"><div><div className="settings-kicker settings-kicker-blue"><span className="settings-dot"/>{data.wado_anonymous ? "WADO STOW-RS" : "DICOMweb STOW-RS"} <span className="settings-kicker-muted">(Entrada / HTTP)</span></div><h2>Endpoint da worklist</h2></div><ConnectionStatus status={stow.status} label={stow.status_label}/></div>
         <p className="settings-card-description">{data.wado_anonymous ? "Endpoint WADO para envio de instâncias DICOM pelo OsiriX, sem usuário ou token." : "Endpoint para envio de instâncias DICOM via requisições multipart autenticadas."}</p>
      ...
      <SettingsCard className="settings-card-security">
         <div className="settings-card-header"><div><div className="settings-kicker settings-kicker-purple"><span className="settings-dot"/>Credenciais para dispositivos</div><h2>Acesso de ingestão</h2></div><ConnectionStatus status={data.wado_anonymous ? "configured" : (data.service_token_configured ? "configured" : "not_configured")} label={data.wado_anonymous ? "WADO sem credencial" : (data.service_token_configured ? "Configurado" : "Não configurado")}/></div>
         <div className="security-content"><div><p className="settings-card-description">{data.wado_anonymous ? "O endpoint WADO do OsiriX está sem usuário e token. O DICOMweb e o gateway externo continuam protegidos." : "Dispositivos que não usam sessão do navegador devem enviar uma credencial de serviço no cabeçalho da requisição."}</p>{!data.wado_anonymous && <div className="credential-methods"><span><LockKeyhole size={13}/>X-DICOM-Ingest-Key</span><span><LockKeyhole size={13}/>Authorization: Bearer</span></div>}</div><div className="security-safe"><ShieldCheck size={18}/><strong>{data.wado_anonymous ? "Modo controlado" : "Valor protegido"}</strong><span>{data.wado_anonymous ? "Use apenas em rede confiável ou com dados desidentificados." : "O segredo nunca é exibido nesta tela."}</span></div></div>
